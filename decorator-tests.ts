const tests: Record<string, () => void> = {
  ...
  'Initializer order': () => {
    ...
    // Field decorators
    const fieldDec1 = (value: undefined, ctxField: ClassFieldDecoratorContext) => {
      log.push('f2')
    ...
    const fieldDec2 = (value: undefined, ctxField: ClassFieldDecoratorContext) => {
      log.push('f1')
      if (!assertEq(() => typeof ctxField.addInitializer, 'function')) return
      ctxField.addInitializer(() => log.push('f3'))
      ctxField.addInitializer(() => log.push('f4'))
    }
    const staticFieldDec1 = (value: undefined, ctxStaticField: ClassFieldDecoratorContext) => {
      log.push('F2')
    ...
    const staticFieldDec2 = (value: undefined, ctxStaticField: ClassFieldDecoratorContext) => {
      log.push('F1')
      if (!assertEq(() => typeof ctxStaticField.addInitializer, 'function')) return
    ...
    // Auto-accessor decorators
    const accessorDec1 = (target: ClassAccessorDecoratorTarget<Foo, undefined>, ctxAccessor: ClassAccessorDecoratorContext) => {
      log.push('a2')
    ...
    const accessorDec2 = (target: ClassAccessorDecoratorTarget<Foo, undefined>, ctxAccessor: ClassAccessorDecoratorContext) => {
      log.push('a1')
      if (!assertEq(() => typeof ctxAccessor.addInitializer, 'function')) return
      ctxAccessor.addInitializer(() => log.push('a3'))
      ctxAccessor.addInitializer(() => log.push('a4'))
    }
    const staticAccessorDec1 = (target: ClassAccessorDecoratorTarget<typeof Foo, undefined>, ctxStaticAccessor: ClassAccessorDecoratorContext) => {
      log.push('A2')
    ...
    const staticAccessorDec2 = (target: ClassAccessorDecoratorTarget<typeof Foo, undefined>, ctxStaticAccessor: ClassAccessorDecoratorContext) => {
      log.push('A1')
      if (!assertEq(() => typeof ctxStaticAccessor.addInitializer, 'function')) return
    ...
    log.push('end')
    assertEq(() => log + '', 'start,' +
      'M1,M2,G1,G2,S1,S2,A1,A2,m1,m2,g1,g2,s1,s2,a1,a2,F1,F2,f1,f2,c1,c2,M3,M4,M5,M6,G3,G4,G5,G6,S3,S4,S5,S6,A3,A4,A5,A6,F3,F4,F5,F6,' +
      'static,' +
      'c3,c4,c5,c6,after,m3,m4,m5,m6,g3,g4,g5,g6,s3,s4,s5,s6,a3,a4,a5,a6,f3,f4,f5,f6,' +
      'ctor,end')
  },
}
